const razorpayConfig = require('../config/rajorpay');
...
exports.capturePayment = async (req, res) => {
    ...
    if (!amountInPaise || amountInPaise < 100) {
        return res.status(400).json({
            success: false,
            message: "Course price must be at least ₹1 to pay via Razorpay",
        });
    }
    ...
    try {
        const paymentResponse = await razorpayConfig.instance.orders.create(options);
        res.status(200).json({
            success: true,
        ...
        console.log("Razorpay order error:", error?.error?.description || error.message);
        return res.status(500).json({
            success: false,
            message: error?.error?.description || "Could not initiate Razorpay order. Check RAZORPAY_KEY and RAZORPAY_SECRET in .env",
        });
    }
